...
export const exportAwsCredentialsFromSsoProfile = (
  ...
  const credOutput = ((): string => {
    try {
      ...
      // .note = ⛔ DIVERGENCE from the declapract best-practice, which passes `{ cause: error }`
      //         raw. `helpful-errors@1.7.3` types `cause?: Error`, and a `catch` binding is
      //         `unknown` under `useUnknownInCatchVariables`, so the raw pass does not compile
      //         here (TS2322). a narrow keeps the real cause on the chain for the common case and
      //         preserves a non-Error throw's message rather than drop it, with no `as` cast
      //         (rule.forbid.as-cast). dispatched upstream.
...
export const useKeyrack = (input?: {
  ...
  // bar imds, so this is keyrack-or-nothing rather than keyrack-or-whatever-the-host-offers.
  // .why = this fn SUPPLIES the right credential; without this line it does not BAR the wrong one.
  //        on any host with an instance role — every ec2 box — the sdk's default chain reaches imds
  //        on its own, with no env var to hint that it did, so a keyrack step that silently no-ops
  //        still yields a credential for the WRONG ACCOUNT. that surfaces as an action-level
  //        AccessDenied which reads exactly like an iam policy gap, and costs hours to tell apart.
  // .note = measured, one probe per state, via `client.config.credentials()`:
  //           AWS_* purged, imds live                      -> ASIATZ2E… (instance role)
  //           AWS_PROFILE set to a NONEXISTENT profile      -> ASIATZ2E… (instance role)
  //           AWS_* purged + AWS_EC2_METADATA_DISABLED=true -> ⛔ CredentialsProviderError
  // .note = ⛔ row 2 is why `mode: 'strict'` cannot stand alone. a SET profile is not a RESOLVED
  //         profile — the chain falls through to imds whenever `fromIni` cannot complete, which
  //         includes an sso session that expires mid-suite (a keyrack grant lasts ~9h). strict
  //         proves the key was granted at t=0 and proves naught about minute 40 of a long run.
  // .note = placed after the `requiresAwsAuth` gate on purpose: it binds only the repos that
  //         declare BOTH a keyrack manifest and an aws account, which is exactly the population
  //         "keyrack or nothing" is a true statement about. a repo with no keyrack returns above,
  //         so its instance role is left alone.
